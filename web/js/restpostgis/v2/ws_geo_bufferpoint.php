 ...
 * @param 		string		$distance		Units to buffer
  * @param 		string		$order		Units to buffer
  * @param 		string		$limit 		Units to buffer
...
try {
	...
	if (strlen(trim($order)) > 0) { $sql .= " order by " . $order; } else { $sql .= " order by Distance(GeomFromText('POINT(" . $x . " " . $y . ")'," . $srid ."),transform(a.the_geom,".$srid."))"; }
