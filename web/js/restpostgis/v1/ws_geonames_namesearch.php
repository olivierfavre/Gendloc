...
try {
	...
	if (strlen($name) < 2) trigger_error("Caught exception: name parameter must include at least two characters.", E_USER_ERROR);
} 
catch (Exception $e) {
...
try {
	...
	elseif ($format == "text") {
		...
		while (!$recordSet->EOF)
		{
			echo $recordSet->fields['name'] . "|" . $recordSet->fields['geonameid'] . "\n";
			$recordSet->MoveNext();
		}
	}
	else {
